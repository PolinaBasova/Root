...
int task9(){  //Only first question about angles
    ...
    TH1D* theta = new TH1D("theta","Theta of photons",45,-4.,4.); //18 bins?
    TH1D* phi = new TH1D("phi","Phi of photons",45,-4.,4.);
    ...
    for (Long64_t ientry = 0; ientry < nentries; ++ientry) {
        ...
        if (ncandidate == 2){
            hM2ph->Fill(GoodPh[0].M());
            hM2ph->Fill(GoodPh[1].M());
            double th = TMath::ACos(GoodPh[0].Pz()/GoodPh[0].E());
            theta->Fill(th);
            phi->Fill( TMath::ASin( GoodPh[0].Px()/( GoodPh[0].E()*TMath::Sin(th) ) ) );
    ...
    theta->GetYaxis()->SetTitle("Number of particles");
    theta->GetXaxis()->SetTitle("Theta");

    phi->GetYaxis()->SetTitle("Number of particles");
